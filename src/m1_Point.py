...
class Point(object):
    ...
    def move_to(self, x, y):
        self.total_distance = self.total_distance + ((self.x - x) ** 2 + (self.y - y) ** 2) ** (1 / 2)
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.total_distance = self.total_distance + (dx ** 2 + dy ** 2) ** (1 / 2)
        self.moves = self.moves + 1
    ...
    def get_distance_from(self, point):
        x_co = self.x - point.x
        y_co = self.y - point.y
        return (x_co ** 2 + y_co ** 2) ** (1 / 2)

    def get_distance_from_start(self):
        x_co = self.x - self.start_x
        y_co = self.y - self.start_y
        return (x_co ** 2 + y_co ** 2) ** (1 / 2)
